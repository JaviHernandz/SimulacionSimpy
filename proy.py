...
def init(self, env, machine, my_id): #constructor que crea el objeto tipo car, necesita env, machine(servidor), id que identifica coche 
self.env = env 
self.id=my_id 
self.server=machine 
...
def arrived_and_washed(self): llegada=randint(1,7)#distribucion de poisson se tiene que poner ahi, para que nos de lambdas de llegada 
...
def running(self): 
#while true yield self.env.process(self.arrived_and_washed()) #capacity numero de servidores que se quieren simular